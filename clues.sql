...
WHERE countrycode = (
    SELECT code
    FROM countries
    WHERE name = 'Country from Clue #1'
)
  AND isofficial = TRUE;
...
WHERE code IN (
    SELECT countrycode
    FROM countrylanguages
    WHERE language = 'Language from Clue #2'
)
  AND code NOT IN (
    SELECT countrycode
    FROM countrylanguages
...
WHERE countrycode = (
    SELECT code
    FROM countries
    WHERE name = 'Country from Clue #3'
)
  AND name != 'Country from Clue #3';
...
FROM cities
WHERE name LIKE 'Pattern%' -- Replace 'Pattern' with part of the name from Clue #4.
  AND name != 'City from Clue #4'
  AND countrycode IN (
    SELECT code
    FROM countries
...
WHERE id = (
    SELECT capital
    FROM countries
    WHERE code = 'Country Code from Clue #5'
);
